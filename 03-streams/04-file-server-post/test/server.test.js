  ...
  describe('тесты на файловый сервер', () => {
    ...
    describe('POST', () => {
      it('возвращается ошибка 409 при создании файла, который есть', done => {
        fse.copyFileSync(path.join(fixturesFolder, 'small.png'), path.join(filesFolder, 'small.png'));

        const mtime = fse.statSync(path.join(filesFolder, 'small.png')).mtime;

        const request = http.request('http://localhost:3001/small.png', { method: 'POST' }, response => {
      ...
      it('успешное создание файла', done => {
        const request = http.request('http://localhost:3001/small.png', { method: 'POST' }, response => {
          expect(response.statusCode, 'статус код ответа сервера 201').to.equal(201);

          expect(fse.existsSync(path.join(filesFolder, 'small.png')), 'файл small.png должен быть на диске').to.be.true;
      ...
      it('файл не должен оставаться на диске при обрыве соединения', done => {
        const request = http.request('http://localhost:3001/example.txt', { method: 'POST' }, response => {
        ...
        request.on('error', err => {
          ...
          setTimeout(() => {
            expect(fse.existsSync(path.join(filesFolder, 'example.txt')), 'файл example.txt не должен оставаться на диске').to.be.false;
      ...
      it('если путь вложенный - возвращается ошибка 400', done => {
        const request = http.request('http://localhost:3001/nested/path', { method: 'POST' }, response => {
