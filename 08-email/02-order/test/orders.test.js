...
describe('email/order', () => {
  ...
  describe('модель заказа', () => {
    ...
    it('должна содержать обязательное свойство `product`', () => {
      ...
      expect(productField.ref, 'свойство product ссылается на модель `Product`').to.be.equal('Product');
    });
  ...
  describe('запрос POST /api/orders должен', () => {
    ...
    it('создать заказ в базе данных, если запрос сформирован корректно', async () => {
      ...
      expect(response.data.order, 'id заказа должен быть валдиным ObjectId').to.satisfy(ObjectId.isValid);
      ...
      expect(order.product.toString(), 'созданный заказ должен содержать переданный продукт').to.equal(body.product.toString());
      expect(order.phone, 'созданный заказ должен содержать переданный номер телефона').to.be.equal(body.phone);
      expect(order.address, 'созданный заказ должен содержать переданный адресс').to.be.equal(body.address);
    });
    ...
    it('отправить письмо пользователю об успешном создании заказа', async () => {
      ...
      expect(get(envelope, 'to[0]'), 'письмо отправлено на почту пользователя').to.equal(user.email);
    });
    ...
    it('использовать id авторизованного пользователя', async () => {
      ...
      expect(order.user.toString(), 'id пользователя должен соответствовать авторизованому,' + 'параметр в теле запроса должен быть проигнорирован').not.be.equal(user.id);
    });
    ...
    it('вернуть ошибку со статус кодом 400 и описанием ' + 'ошибки валидации при неправильно сформированном запросе', async () => {
      ...
      expect(data.errors, 'products - ожидается получить ObjectId').to.have.property('product').that.include('required');
      expect(data.errors, 'phone - свойство должно соответствовать шаблону').to.have.property('phone').that.equal('Неверный формат номера телефона.');
      expect(data.errors, 'address - свойство обязательно').to.have.property('address').that.include('required');
    });
  ...
  describe('запрос GET /api/orders должен', () => {
    ...
    it('вернуть список заказов текущего пользователя', async () => {
      ...
      expect(data.orders, 'ответ должен содержать только заказы текущего пользователя').to.satisfy(() => data.orders.every(order => (order.user = user.id)));
    });
